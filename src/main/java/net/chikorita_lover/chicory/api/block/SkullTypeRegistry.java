...
public final class SkullTypeRegistry {
    ...
    public static SkullBlock.Type register(Identifier id) {
        try {
            ...
            ChicoryApi.LOGGER.error("Could not register skull type {}: {}", id, e.getMessage());
            return SkullBlock.Type.SKELETON;
        }
    }

    public static SkullBlock.Type register(Identifier id, EntityType<?>... entities) {
        final SkullBlock.Type type = register(id);
    ...
    @ApiStatus.Internal
    public static Item registerItem(SkullBlock.Type type, Item item) {
    ...
    public static Identifier getId(final SkullBlock.Type type) {
        if (!contains(type)) {
            ChicoryApi.LOGGER.warn("Attempted to get identifier of skull type {} not registered through Chicory API", type.name());
            return DEFAULT_ID;
        }
